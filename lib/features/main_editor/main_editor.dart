...
class ProImageEditorState extends State<ProImageEditor>
  ...
  late final LayerInteractionManager layerInteractionManager = LayerInteractionManager(
  ...
  void removeLayer(Layer? layer) {
    int layerPos = activeLayers.indexWhere((element) => element.id == (layer?.id ?? _tempLayer!.id));
    if (layerPos >= 0) {
      stateManager.activeLayers[layerPos] = _layerCopyManager.copyLayer(_tempLayer ?? layer!);

      mainEditorCallbacks?.handleRemoveLayer(stateManager.activeLayers[layerPos]);
  ...
  void _updateTempLayer() {
    ...
    List<Layer> oldLayers = stateHistory[stateManager.historyPointer - 1].layers;
  ...
  void _initializeVideoEditor() async {
    ...
    Future<Uint8List> createTransparentImage(double width, double height) async {
  ...
  Future<void> decodeImage([
    ...
    bool shouldImportStateHistory = _isImageNotDecoded && stateHistoryConfigs.initStateHistory != null;
    _isImageNotDecoded = false;
  ...
  void _calcAppBarHeight() {
    ...
    if (renderedBottomBarHeight != null) {
      sizesManager
        ...
        ..appBarHeight =
            sizesManager.editorSize.height - sizesManager.bodySize.height - sizesManager.bottomBarHeight;
    }
  }
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    if (layerInteractionManager.rotateScaleLayerSizeHelper != null) {
      layerInteractionManager
        ..freeStyleHighPerformanceScaling = paintEditorConfigs.enableFreeStyleHighPerformanceScaling ?? !isDesktop
        ..calculateInteractiveButtonScaleRotate(
          ...
          editorScaleOffset: interactiveViewer.currentState?.offset ?? Offset.zero,
        );
    ...
    double editorScaleFactor = interactiveViewer.currentState?.scaleFactor ?? 1.0;

    layerInteractionManager.enabledHitDetection = false;
    if (details.pointerCount == 1) {
      layerInteractionManager
        ..freeStyleHighPerformanceMoving = paintEditorConfigs.enableFreeStyleHighPerformanceMoving ?? isWebMobile
        ..calculateMovement(
  ...
  Future<T?> openPage<T>(
    ...
    return Navigator.push<T?>(
      ...
      PageRouteBuilder(
        ...
        barrierDismissible: mainEditorConfigs.style.subEditorPage.barrierDismissible,
        transitionDuration: duration,
        reverseTransitionDuration: duration,
        transitionsBuilder: mainEditorConfigs.style.subEditorPage.transitionsBuilder ??
              ...
              child: Stack(
                ...
                children: [
                  Positioned(
                    ...
                    child: Center(
                      child: Container(
                        width: mainEditorConfigs.style.subEditorPage.enforceSizeFromMainEditor
                            ? sizesManager.editorSize.width
                            : null,
                        height: mainEditorConfigs.style.subEditorPage.enforceSizeFromMainEditor
                            ? sizesManager.editorSize.height
                            : null,
                        clipBehavior: Clip.hardEdge,
                        decoration: BoxDecoration(
                          borderRadius: mainEditorConfigs.style.subEditorPage.borderRadius,
                        ),
                        child: page,
                      ),
                    ),
  ...
  void openPaintEditor() async {
    var paintCallbacks = callbacks.paintEditorCallbacks ?? const PaintEditorCallbacks();
    ...
    List<PaintLayer>? paintItemLayers = await openPage<List<PaintLayer>>(
      PaintEditor.autoSource(
        ...
        initConfigs: PaintEditorInitConfigs(
          configs: configs,
          callbacks: callbacks.copyWith(paintEditorCallbacks: overridenPaintCallbacks),
          layers: activeLayers,
          theme: _theme,
  ...
  void openEmojiEditor() async {
    ...
    _checkInteractiveViewer();
    ServicesBinding.instance.keyboard.removeHandler(_onKeyEvent);
    final effectiveBoxConstraints = emojiEditorConfigs.style.editorBoxConstraintsBuilder?.call(context, configs);

    DraggableSheetStyle sheetTheme = emojiEditorConfigs.style.themeDraggableSheet;
    ...
    EmojiLayer? layer = await showModalBottomSheet(
        ...
        builder: (BuildContext context) => SafeArea(
              child: !useDraggableSheet
                  ? ConstrainedBox(
                      constraints: effectiveBoxConstraints ??
                          BoxConstraints(maxHeight: 300 + MediaQuery.viewInsetsOf(context).bottom),
  ...
  void openStickerEditor() async {
    ...
    _checkInteractiveViewer();
    ServicesBinding.instance.keyboard.removeHandler(_onKeyEvent);
    final effectiveBoxConstraints = stickerEditorConfigs.style.editorBoxConstraintsBuilder?.call(context, configs);
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (callbacks.onThumbnailGenerated != null) {
        ...
        final results = await Future.wait([
          captureEditorImage(),
          _controllers.screenshot.getRawRenderedImage(imageInfos: _imageInfos!, useThumbnailSize: false),
        ]);
  ...
  Future<Uint8List> captureEditorImage() async {
    ...
    bool useOriginalImage = !_isVideoEditor && !hasChanges && imageGenerationConfigs.enableUseOriginalBytes;
  ...
  Widget build(BuildContext context) {
    ...
    return RecordInvisibleWidget(
      ...
      child: ExtendedPopScope(
        canPop: isPopScopeDisabled || !stateManager.canUndo || _isProcessingFinalImage,
        onPopInvokedWithResult: (didPop, result) {
          if (!didPop && !isPopScopeDisabled && stateManager.canUndo && !_isProcessingFinalImage) {
            closeWarning();
          }
        ...
        child: ImageInfosProvider(
          infos: _imageInfos,
          imageFitToWidth: _imageInfos?.renderedSize.width == sizesManager.bodySize.width,
                    ...
                    return Scaffold(
                      ...
                      body: Padding(
                        ...
                        child: Stack(
                          children: [
                            _buildBody(),
                            _buildBottomNavBar() ?? const SizedBox(),
                          ],
                        ),
                      ),
                      // body: _buildBody(),
  ...
  PreferredSizeWidget? _buildAppBar() {
    if (mainEditorConfigs.widgets.appBar != null) {
      return mainEditorConfigs.widgets.appBar!.call(this, _rebuildController.stream);
    }
  ...
  Widget? _buildBottomNavBar() {
    if (mainEditorConfigs.widgets.bottomBar != null) {
      return mainEditorConfigs.widgets.bottomBar!.call(this, _rebuildController.stream, _bottomBarKey);
    }
  ...
  Widget _buildImage() {
    return MainEditorBackgroundImage(
      backgroundImageColorFilterKey: _isVideoEditor ? GlobalKey() : _backgroundImageColorFilterKey,
